...
def compose_paths(component_paths):
    return [
        compose for path in component_paths if os.path.isfile(compose := os.path.join(path, "docker-compose-extra.yml"))
    ]
...
class TestDockerCompose:
    ...
    def test_logging(self, compose_paths):
        ...
        assert (
            not bad_logging
        ), f"services should all use the following logging configuration {good_logging_config}: errors: \n {errors}"
