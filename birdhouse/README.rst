...
.. warning::
    **Do not create tags or releases manually for this repository.**
    ...
    Be sure to consult the following guidelines before proceeding.

We aim to follow the standard of `semantic versioning <https://semver.org/>`_.
...
Once the version as been bumped and the PR is merged, a corresponding version tag will be added
to the commit generated by the merge automatically via GitHub Workflows.  This step is
intentionally automated to ensure that there are less opportunities for human error when
...
* Only when you are ready to merge the PR immediately, you can continue with
  ...
  * Once merged to ``master`` the ``create-tag.yml`` and ``release.yml`` workflows will run.
    ...
    creates a new release with an autogenerated summary of changes. 
