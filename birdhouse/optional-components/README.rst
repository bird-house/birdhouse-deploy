...
in order to test secured access this service before it is moved behind Twitcher (undetermined date).

The old ``/geoserver`` path is still available, so current workflows are not be affected.
...
permissions can be set on a per-workspace or even layer basis. See 
